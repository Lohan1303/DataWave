    ...
    try {
      ...
      for (let t = intervaloInicial; t <= intervaloFinal; t += passo) {
        ...
        for (let n = 1; n <= qtdHarmonicas; n++) {
          ...
          somaHarmonicas_1 += -A_n_1 * Math.sin(2 * Math.PI * n * frequenciaFundamental * (t - passo));
          somaHarmonicas_2 += -A_n_2 * Math.sin(2 * Math.PI * n * frequenciaFundamental * (t + passo));
        }

        // Armazenando os valores em X e Y
        setCoordY((prevCoordY) => [...prevCoordY, somaHarmonicas]);

        if (somaHarmonicas_1 > somaHarmonicas && somaHarmonicas <= somaHarmonicas_2) {
